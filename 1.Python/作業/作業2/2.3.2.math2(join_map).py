...
def topic1(n):
    ...
    for i in range(1, n+1):
        sum+=1/i
        s=f'{i}' if i==1 else f'+1/{i}' if i%2==1 else f'+1/{i}' 
        lst.append(s)
    st="".join(lst[:4])+'...+'+lst[-1]+"="
    return sum, st
...
def topic4(n):
    ...
    for i in range(1, n+1):
        ...
        for j in range(1,i+1):
            sum1+=1/j
            s1=f'{j}' if j==1 else f'1/{j}'
            ls.append(s1)
        sum+=sum1
        s="+".join(ls)              #連ls的各項
        s="("+s+")" if i>1 else s #加"1/"及前後小括號
        lst.append(s)               #加入lst，ls即可清除
        ls.clear()
    #print(f'lst={lst}')    
    s1=lst[3]   #lst的第4小項 s1='(1+1/2+1/3+1/4)'
    st="+".join(lst[:4])+'...+'+s1[:-1]+f'...+1/{n})='
    return sum, st
